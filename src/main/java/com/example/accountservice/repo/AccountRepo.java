...
public interface AccountRepo extends JpaRepository<AccountDTO, Integer> {
    ...
    @Query(value = "SELECT * FROM accountdto WHERE email = :email", nativeQuery = true)
    List<AccountDTO> findAllPathFile(String email);
    ...
    @Query(value = "SELECT * FROM accountdto", nativeQuery = true)
